...
campaign_url <- function(url,source,medium=NULL,campaign_name=NULL,term=NULL,content=NULL) {

  require(purrr)

final_url <- paste(url,'/?utm_source=',source,sep='')

if (is_empty(medium) == FALSE) {

  final_url <- paste(final_url,'&utm_medium=',medium,sep='')
